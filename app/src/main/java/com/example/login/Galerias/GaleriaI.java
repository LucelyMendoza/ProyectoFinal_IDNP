...
public class GaleriaI extends View {

    private Canvas _canvas;
    private List<PointRoom> points;
    private Context context;

    public GaleriaI(Context context) {
        super(context);
        init(context);
    }

    public GaleriaI(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public GaleriaI(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawRoom() {
        ...
        float roomHeight = _canvas.getHeight() * 0.3f; // Ajustar la altura del rectángulo
        ...
        // Línea interna vertical
        _canvas.drawLine(left + roomWidth * 0.25f, top, left + roomWidth * 0.25f, top + 250, paint); // Línea vertical 1
        _canvas.drawLine(left + roomWidth * 0.25f, top + 500, left + roomWidth * 0.25f, bottom, paint); // Línea vertical 2
    }

    private void drawPictures() {
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setStrokeWidth(30f); // Ajustar el grosor de las líneas moradas
        paint.setStyle(Paint.Style.STROKE);
    ...
    private void drawText() {
        ...
        _canvas.drawText("Galería I", centerX, centerY + 30, paint); // Ajustar la posición del texto si es necesario
    }
}
